...
//        },
    ...
    //4.编码器
    OSStatus result = AudioConverterNewSpecific(&inputFormat, &outputFormat, outputFormat.mChannelsPerFrame, requestedCodecs, &converterRef);
...
- (void)encodeWithBufferList:(AudioBufferList)bufferList completianBlock:(void (^)(NSData *encodedData, NSError *error))completionBlock{
    ...
    OSStatus status = AudioConverterFillComplexBuffer(converterRef,
                                                      inputDataProc,
                                                      &bufferList,
...
- (NSData *)getADTSDataWithPacketLength:(NSInteger)packetLength {
    ...
    int freqIdx = 4;  //44.1KHz 4 8KHz 11
